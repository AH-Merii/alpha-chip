...
class GrlPolicyModel(network.DistributionNetwork):
    ...
    def call(self, inputs, step_types=None, network_state=()):
        ...
        paddings = tf.ones_like(inputs["mask"], dtype=tf.float32) * (-(2.0**32) + 1)
        ...
        output_dist = self._output_dist_spec.build_distribution(logits=masked_logits)
...
class GrlValueModel(network.Network):
    ...
    def __init__(
        ...
        super().__init__(input_tensor_spec=input_tensors_spec, state_spec=(), name=name)
