...
class CircuitTrainingModel(tf.keras.layers.Layer):
    ...
    def __init__(
        self,
        all_static_features: dict[str, np.ndarray] | None = None,
        ...
        self._edge_fc_list = [
            create_edge_fc(name="edge_fc_%d" % i) for i in range(self._num_gcn_layers)
        ]
    ...
    def _scatter_count(
        ...
        h_node = tf.zeros([batch, self._observation_config.max_num_nodes, num_lattents])
    ...
    def _get_static_input(
        ...
        return tf.gather(self._all_static_features[static_feature_key], netlist_index)
    ...
    def call(
        ...
        finetune_value_only = tf.convert_to_tensor(finetune_value_only, dtype=tf.bool)
