...
from circuit_training.environment import (
    observation_config,
...
class InfeasibleActionError(ValueError):
    ...
    def __str__(self):
        return f"Infeasible action ({self.action}) when the mask is ({self.mask})"
...
def cost_info_function(
    ...
    proxy_cost = 0.0
    info = dict.fromkeys(COST_COMPONENTS, -1.0)
...
class CircuitEnv:
    ...
    def _get_mask(self) -> np.ndarray:
        ...
        return np.reshape(mask, (self._observation_config.max_grid_size**2,)).astype(
            np.int32
        )
    ...
    def analytical_placer(self) -> None:
        ...
        if self._std_cell_placer_mode == "dreamplace":
            ...
            raise ValueError(
                f"{self._std_cell_placer_mode} is not a supported std_cell_placer_mode."
            )
