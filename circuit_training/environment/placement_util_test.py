...
_TESTDATA_DIR = _CIRCUIT_TRAINING_DIR + "/environment/test_data/macro_tiles_10x10"


class MockPlacementCost:
...
class PlacementUtilTest(test_utils.TestCase):
    def setUp(self):
        netlist_file = os.path.join(FLAGS.test_srcdir, _TESTDATA_DIR, "netlist.pb.txt")
        init_placement = os.path.join(FLAGS.test_srcdir, _TESTDATA_DIR, "initial.plc")
    ...
    def test_save_placement(self):
        ...
        self.assertEqual(
            placement_util.extract_attribute_from_comments("Wirelength", [filename]),
            "11",
        )
        self.assertEqual(
            placement_util.extract_attribute_from_comments(
        ...
        self.assertEqual(
            placement_util.extract_attribute_from_comments("Density cost", [filename]),
            "14",
        )
        self.assertEqual(
    ...
    def test_blockage_with_spacing_constraints(self):
        blockages = placement_util.create_blockages_by_spacing_constraints(10, 20, 1, 2)
        expected_blockages = [
            [0, 0, 1, 20, 0.1],
            [9, 0, 10, 20, 0.1],
            [0, 0, 10, 2, 0.1],
