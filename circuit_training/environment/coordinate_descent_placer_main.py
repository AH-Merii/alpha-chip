...
_NETLIST_FILE = flags.DEFINE_string("netlist_file", None, "Path to netlist file.")
_INIT_PLACEMENT = flags.DEFINE_string(
    "init_placement", None, "Path to initial placement file."
)
_CD_OUTPUT_DIR = flags.DEFINE_string("cd_output_dir", "/tmp/cd", "CD output dir.")
...
def main(_):
    ...
    placer.place()
    output_plc_file = os.path.join(_CD_OUTPUT_DIR.value, _CD_PLACEMENT_FILENAME.value)
