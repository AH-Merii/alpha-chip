...
def extract_attribute_from_comments(attribute: str, filenames: list[str]) -> str | None:
...
def create_placement_cost(
    ...
    blockages: list[list[float]] | None = None,
    fixed_macro_names_regex: list[str] | None = None,
...
def get_ordered_node_indices(
    ...
    if mode == "legalization_order":
        ...
        nodes = {nm[0] for _, nm in fake_nets}.union(
            {nm[1] for _, nm in fake_nets}
        ).union(set(hard_macro_indices))
        is_port = {n: n not in hard_macro_indices for n in nodes}
        macro_with_fake_net = dict.fromkeys(nodes, False)
        ...
        closeness = dict.fromkeys(nodes, 0.0)
        ...
        raise ValueError(f"{mode} is an unsupported node placement mode.")
...
def disconnect_high_fanout_nets(
    ...
    for i in nodes_of_types(plc, ["PORT", "STDCELL", "MACRO_PIN"]):
        ...
        if num_fanouts > max_allowed_fanouts:
            print(
                f"Disconnecting node: {plc.get_node_name(i)} with {num_fanouts} fanouts."
            )
...
def legalize_placement(plc: plc_client.PlacementCost) -> bool:
    ...
    for node in ordered_nodes:
        ...
        if node in previous_xy_coords and not plc.is_node_soft_macro(node):
            ...
            print(
                f"x/y displacement: dx = {x - px}, dy = {y - py}, macro: {plc.get_node_name(node)}"
            )
            total_macro_displacement += abs(x - px) + abs(y - py)
            total_macros += 1
    print(
        f"Total macro displacement: {total_macro_displacement}, avg: {total_macro_displacement / total_macros}"
    )
    return True
...
def create_blockages_by_spacing_constraints(
    ...
    rectilinear_blockages: list[list[float]] | None = None,
) -> list[list[float]]:
