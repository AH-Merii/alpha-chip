...
FeatureKeyType = Union[list[str], tuple[str, ...]]
...
STATIC_OBSERVATIONS = (
    NETLIST_METADATA + GRAPH_ADJACENCY_MATRIX + NODE_STATIC_FEATURES + NETLIST_INDEX
)
...
def _flatten(dict_obs: dict[str, TensorType], keys: FeatureKeyType) -> TensorType:
    out = [np.asarray(dict_obs[k]) for k in keys]
    return np.concatenate(out, axis=-1)


def flatten_static(dict_obs: dict[str, TensorType]) -> TensorType:
    return _flatten(dict_obs=dict_obs, keys=STATIC_OBSERVATIONS)


def flatten_dynamic(dict_obs: dict[str, TensorType]) -> TensorType:
    return _flatten(dict_obs=dict_obs, keys=DYNAMIC_OBSERVATIONS)


def flatten_all(dict_obs: dict[str, TensorType]) -> TensorType:
