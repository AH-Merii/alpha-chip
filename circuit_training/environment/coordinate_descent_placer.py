...
from circuit_training.dreamplace import dreamplace_core, dreamplace_util
...
@gin.configurable
class CoordinateDescentPlacer:
    """Coordinate descent algorithm to place nodes."""

    def __init__(
        self,
        plc: plc_client.PlacementCost,
        cost_fn: Callable[[plc_client.PlacementCost], tuple[float, dict[str, float]]],
        epochs: int = 2,
        ...
        for node in self.plc.get_macro_indices():
            if not self.plc.is_node_soft_macro(node):
                ...
                if cur_ori in NS_ORIENTATIONS:
                    ...
                    raise ValueError(
                        f"Unexpected orientation {cur_ori} for node {node}."
                    )
    ...
    def find_best_orientation(self, node: int, orientations: list[str]) -> str | None:
    ...
    def place_stdcells(self) -> None:
        ...
        if self._stdcell_placer == "fd":
            ...
            raise ValueError(f"stdcell placer {self._stdcell_placer} is not supported")
