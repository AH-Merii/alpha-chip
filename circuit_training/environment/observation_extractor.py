...
class ObservationExtractor:
    ...
    def __init__(
        ...
        observation_config: observation_config_lib.ObservationConfig | None = None,
        netlist_index: int = 0,
    ...
    def _extract_static_features(self) -> dict[str, np.ndarray]:
    ...
    def _extract_num_macros(self, features: dict[str, np.ndarray]) -> None:
        features["num_macros"] = np.asarray([len(self.plc.get_macro_indices())]).astype(
            np.int32
        )

    def _extract_technology_info(self, features: dict[str, np.ndarray]) -> None:
    ...
    def _extract_initial_node_locations(self, features: dict[str, np.ndarray]) -> None:
    ...
    def _extract_node_types(self, features: dict[str, np.ndarray]) -> None:
        """Extracts node types."""
        types = []
        for macro_idx in self.plc.get_macro_indices():
    ...
    def _extract_macro_size(self, features: dict[str, np.ndarray]) -> None:
    ...
    def _extract_macro_and_port_adj_matrix(
        ...
        features["sparse_adj_weight"] = np.asarray(sparse_adj_weight).astype(np.float32)
        features["edge_counts"] = edge_counts

    def _extract_canvas_size(self, features: dict[str, np.ndarray]) -> None:
    ...
    def _extract_grid_size(self, features: dict[str, np.ndarray]) -> None:
    ...
    def _extract_netlist_index(self, features: dict[str, np.ndarray]) -> None:
        features["netlist_index"] = np.asarray([self._netlist_index]).astype(np.int32)
    ...
    def _add_netlist_metadata(self, features: dict[str, np.ndarray]) -> None:
    ...
    def _normalize_adj_matrix(self, features: dict[str, np.ndarray]) -> None:
        """Normalizes adj matrix weights."""
        mean_weight = np.mean(features["sparse_adj_weight"])
        features["sparse_adj_weight"] = (
            features["sparse_adj_weight"] / (mean_weight + ObservationExtractor.EPSILON)
        ).astype(np.float32)
    ...
    def _pad_adj_matrix(self, features: dict[str, np.ndarray]) -> None:
    ...
    def _pad_macro_static_features(self, features: dict[str, np.ndarray]) -> None:
    ...
    def _pad_macro_dynamic_features(self, features: dict[str, np.ndarray]) -> None:
        """Pads macro features to make their shape knwon."""
        for var in [
            "locations_x",
            "locations_y",
    ...
    def _normalize_canvas_size_by_canvas(self, features: dict[str, np.ndarray]) -> None:
    ...
    def _normalize_grid_size(self, features: dict[str, np.ndarray]) -> None:
        features["grid_cols"] = (
    ...
    def _normalize_macro_size_by_canvas(self, features: dict[str, np.ndarray]) -> None:
        """Normalizes macro sizes with the canvas size."""
        features["macros_w"] = (features["macros_w"] / (self.half_perimeter)).astype(
            np.float32
        )
        features["macros_h"] = (features["macros_h"] / (self.half_perimeter)).astype(
            np.float32
        )

    def _normalize_locations_by_canvas(self, features: dict[str, np.ndarray]) -> None:
    ...
    def get_fake_net_heatmap(self, current_node_index: int) -> np.ndarray:
        ...
        costs = np.sum(
            weights[:, np.newaxis]
            * self._grid_l2_distance(related_node_locs[:, np.newaxis], all_locations),
            axis=0,
        )
    ...
    def get_static_features(self) -> dict[str, np.ndarray]:
        return {
    ...
    def _update_dynamic_features(
        ...
        if previous_node_index >= 0:
            ...
            self._features["locations_x"][previous_node_index] = x / self.half_perimeter
            self._features["locations_y"][previous_node_index] = y / self.half_perimeter
            self._features["is_node_placed"][previous_node_index] = 1
        self._features["fake_net_heatmap"] = self.get_fake_net_heatmap(
            current_node_index
        )
        self._features["packing_heatmap"] = self.get_packing_heatmap(current_node_index)
