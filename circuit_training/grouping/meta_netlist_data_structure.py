...
class NetlistNode:
    ...
    output_indices: list[int] | None = dataclasses.field(default_factory=list)
    input_indices: list[int] | None = dataclasses.field(default_factory=list)

    # If this node is a macro, use this boolean to check if it's a soft (made up
    # of standard cells), or hard (like an SRAM).
    soft_macro: bool | None = False
