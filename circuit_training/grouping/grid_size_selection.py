...
flags.DEFINE_float("tolerance", 0.05, "Tolerance to choose lower number of grids")
...
def get_grid_suggestion(plc):
    ...
    if not choices:
        return None, None
    cols, rows = select_from_grid_choices(choices, FLAGS.tolerance, print_best_n=20)
    return cols, rows
...
def is_num_grid_cells_acceptable(
    ...
    return num_grid_cells <= max_num_grid_cells and num_grid_cells >= min_num_grid_cells
...
def select_from_grid_choices(grid_choices, tolerance=0.1, print_best_n=0):
    ...
    assert grid_choices
    sorted_list = sorted(
        grid_choices.items(), key=lambda x: x[1].key_metric, reverse=True
    )
    ...
    qualified = [
        (k, v) for (k, v) in sorted_list if v.key_metric >= best_key_metric_tolerance
    ]
