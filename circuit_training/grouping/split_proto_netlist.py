...
def split_proto_netlist(
    ...
    print("-------------------")
    print(f"Max file size = {int(max_file_size / (1024 * 1024))}MB")
    print(f"Print interval = {int(print_pos_interval / (1024 * 1024))}MB")
    ...
    outfile_cnt = 1
    outfile_name = f"{file_name_base}.part{outfile_cnt}.pb.txt"
    ...
    with gfile.GFile(file_name, "r") as infile:
        for line in infile:
            ...
            if infile_pos >= next_print_pos:
                print(f"Reading input file at {int(infile_pos / (1024 * 1024))}MB")
                next_print_pos += print_pos_interval
