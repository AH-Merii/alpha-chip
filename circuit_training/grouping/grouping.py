...
class Grouping:
    ...
    def get_side(self, x: float, y: float, width: float, height: float) -> mnds.Side:
    ...
    def group_ids(self) -> list[int]:
        """Groups ids."""
        return sorted(self._node_groups)
    ...
    def set_node_group(self, node_index: int, group_index: int) -> None:
        ...
        self._max_group_id = max(self._max_group_id, group_index)
    ...
    def write_as_macro(self, group_no: int, graph_def: tf.compat.v1.GraphDef) -> None:
        ...
        for node_index in self._node_groups[group_no]:
            ...
            if len(node_fanout) == 1:
                key, value = next(iter(node_fanout.items()))
            ...
            weight = next(iter(node_fanout.items()))[1]
            if weight != 1.0:
    ...
    def merge_small_adj_close_groups(self, max_num_nodes: int, distance: float) -> bool:
