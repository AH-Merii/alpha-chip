...
from circuit_training.environment import placement_util, plc_client
from circuit_training.grouping import (
    grid_size_selection,
    grouping,
    hmetis_util,
...
flags.DEFINE_integer("num_groups", 500, "Number of std-cell groups (soft macros)")
...
def write_new_netlist(
    ...
    user_comments = (
        f"Original source netlist with standard cells: {plc.get_source_filename()}\n"
    )
    user_comments += f"Groups file: {final_groups_file}\n"
...
def get_new_output_dir(ngrps: int, hopts: Any) -> str:
    return f"g{ngrps}_ub{hopts.ub_factor}_nruns{hopts.n_runs}_c{hopts.c_type}_r{hopts.r_type}_v{hopts.v_cycle}_rc{hopts.reconst}"
...
def run_with_default_hmetis_options(
    ...
    new_output_dir = os.path.join(output_dir, get_new_output_dir(num_groups, hopts))
...
def update_groups_using_metis_output(grp: grouping.Grouping, metis_out_file: str):
...
def worst_spread_metrics_log(grp: grouping.Grouping, num_worst: int = 10) -> str:
    ...
    sorted_list = sorted(
        grp_spread.items(), key=lambda kv: (kv[1], kv[0]), reverse=True
    )
    result = f"worst {num_worst} spread\n"
    for v in sorted_list[0:num_worst]:
        result += f"grp: {v[0]} - spread: {v[1]}\n"
    return result
...
def group_stdcells(
    ...
    output_dir = os.path.join(output_dir, f"{cols}cols_{rows}rows")
    ...
    placement_util.save_placement(
        grouped_plc,
        legalized_placement,
        f"Original file : {netlist_file}\nInitial placement : {plc_file}\n",
    )
