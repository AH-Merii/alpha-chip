...
from tf_agents.metrics import py_metric, py_metrics
...
from circuit_training.learning import agent, static_feature_cache
...
class PlacementImage(py_metric.PyStepMetric):
    ...
    def __init__(self, num_rows: int, num_cols: int):
        ...
        """
        super().__init__("placement_image")
...
class FirstPolicyImage(py_metric.PyStepMetric):
    ...
    def __init__(self, num_rows: int, num_cols: int, actor_net: network.Network):
        ...
        Args:
          ...
          actor_net: Actor net.
        """
        super().__init__("first_policy_image")
...
class InfoMetric(py_metric.PyStepMetric):
    ...
    def __init__(
        ...
        """
        super().__init__(name + "_" + info_metric_key)
...
def evaluate(
    ...
    info_metric_names: list[str] | None = None,
    summary_subdir: str = "",
):
    ...
    eval_actor = actor.Actor(
        ...
        train_step,
        episodes_per_run=1,
        summary_dir=os.path.join(root_dir, learner.TRAIN_DIR, summary_subdir, "eval"),
        metrics=[
            py_metrics.NumberOfEpisodes(),
            py_metrics.EnvironmentSteps(),
            py_metrics.AverageReturnMetric(name="eval_episode_return", buffer_size=1),
    ...
    while True:
        logging.info("Evaluating using greedy policy at step: %d", train_step.numpy())
        eval_actor.run()
