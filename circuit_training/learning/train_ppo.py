...
from circuit_training.learning import static_feature_cache, train_ppo_lib
...
_GIN_BINDINGS = flags.DEFINE_multi_string("gin_bindings", [], "Gin binding parameters.")
...
def read_policy(
    ...
    checkpoints_prefix = os.path.join(policy_checkpoint_dir, "variables", "variables")
...
def load_policy_checkpoints(
    ...
    policy_variable_map = read_policy(policy_saved_model_dir, policy_checkpoint_dir)
...
def try_load_checkpoint(
    ...
    actor_net: tf.keras.layers.Layer,
    value_net: tf.keras.layers.Layer,
    policy_saved_model_dir: str | None = None,
    ...
    current_saved_model_dir = os.path.join(root_dir, learner.POLICY_SAVED_MODEL_DIR)
...
def main(_):
    ...
    assert len(_NETLIST_FILE.value) == len(_INIT_PLACEMENT.value), (
