...
def unplaceable_area(plc):
    return np.sum([(b[2] - b[0]) * (b[3] - b[1]) for b in get_relevent_blockages(plc)])


def np_array_of_array(py_list_of_list, dtype):
    """converts a Python list of list into a Numpy array of array."""
    return np.array([np.array(l, dtype=dtype) for l in py_list_of_list], dtype=object)
...
def convert_nodes(db, plc, hard_macro_order):
    ...
    physical_node_indices = (
        soft_macro_and_stdcell_indices + hard_macro_indices + non_movable_node_indices
    )
    ...
    node_index_to_node_id_map = {n: i for i, n in enumerate(physical_node_indices)}
...
def convert_pins_and_nets(db, plc, physical_node_indices, node_index_to_node_id_map):
...
def convert_to_ndarray(db):
    ...
    db.node_names = np.array(db.node_names, dtype=np.bytes_)
    db.node_x = np.array(db.node_x, dtype=db.dtype)
    db.node_y = np.array(db.node_y, dtype=db.dtype)
    db.node_orient = np.array(db.node_orient, dtype=np.bytes_)
    ...
    db.pin_direct = np.array(db.pin_direct, dtype=np.bytes_)
    ...
    db.net_names = np.array(db.net_names, dtype=np.bytes_)
...
class PlcConverter:
    ...
    def non_movable_macro_area(self, plc, num_non_movable_macros=None):
        ...
        macro_width_height = [plc.get_node_width_height(m) for m in non_movable_macro]
    ...
    def update_num_non_movable_macros(self, db, plc, num_non_movable_macros):
        ...
        macro_mask = [False] * len(self._soft_macro_and_stdcell_indices) + [True] * (
